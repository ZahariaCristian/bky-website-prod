...
const PANEL_PLATFORM = "trovagnocca";

const EDIT = URL_PARAMS.get("enableEdit");
var blacklist = [];
var tmpID = 0;
var sCalendar = null;
const cityField = () => document.querySelector("input[name='city'], select[name='city']");
...
const setSelectValueByValueOrText = (selector, value) => {
    ...
    const option = Array.from(select.options).find((item) => {
        const optionText = `${item.textContent || ""}`.replace(/[^\p{L}\p{N}_ ]/gu, "").trim().toLowerCase();
...
function deleteStorico(me, idString) {
    ...
    if (id) {
        fetch("/annuncio/deleteStorico", {
            ...
            redirect: "follow",
            referrerPolicy: "no-referrer",
            body: JSON.stringify({ id: id, annuncio: anID, panel: PANEL_PLATFORM })
        }).then((r) => {
            toggleLoader();
            if (r.status == 401) {
                window.location.href = "/";
...
const cloneDate = document.querySelector("#txtDateClona");
if (cloneDate) cloneDate.value = new Date(tmpToday).toLocaleDateString("zh-hans-cn", {
    year: "numeric",
    month: "2-digit",
    day: "2-digit",
}).replace(/\//g, "-");
const cloneDateFrom = document.querySelector("#txtDateClonaFrom");
if (cloneDateFrom) cloneDateFrom.value = new Date(tmpToday).toLocaleDateString("zh-hans-cn", {
    year: "numeric",
    month: "2-digit",
    day: "2-digit",
}).replace(/\//g, "-");
tmpToday.setDate(tmpToday.getDate() + 1);
const cloneDateTo = document.querySelector("#txtDateClonaTo");
if (cloneDateTo) cloneDateTo.value = new Date(tmpToday).toLocaleDateString("zh-hans-cn", {
    year: "numeric",
    month: "2-digit",
    day: "2-digit",
}).replace(/\//g, "-");
...
document.querySelector("#caricalink-button").addEventListener("click", () => {
    ...
    fetch("/annuncio/scrapeTrovagnocca", {
        ...
        r.json().then((res) => {
            if (res.donna == null) {
                return window.location = "/annuncio.html?edit=" + res.id + "&panel=" + PANEL_PLATFORM + "&enableEdit=true";
            }
            window.location = "/annuncio.html?edit=" + res.id + "&panel=" + PANEL_PLATFORM;
        });
    });
});
...
document.querySelector("#caricaphone-button").addEventListener("click", () => {
    ...
    fetch("/annuncio/getByPhone", {
        ...
        redirect: "follow",
        referrerPolicy: "no-referrer",
            body: JSON.stringify({ phone, key: localStorage.getItem("key"), panel: PANEL_PLATFORM }),
    }).then((r) => {
        if (r.status !== 200) return alert(
            "⚠ Si è verificato un errore durante il caricamento dati da URL",
        );
        r.json().then(async (res) => {
            ...
            document.querySelector("div.container h1").innerHTML = "<i>Gestisci l'annuncio-Trovagnocca</i>";
        });
    });
});
...
async function loadAdvertisement(res) {
    ["title", "age", "description", "phone", "location", "name", "donnaID"].forEach((info) => {
        const field = document.querySelector(`${(info == "description" || info == "title") ? "textarea" : "input"}[name='${info}']`);
    ...
    if (res.phone || res.isPhoneChecked) phoneApprove();
...
function updateInfo(save) {
    ...
    fetch("/annuncio/updateInfo", {
        ...
        redirect: "follow",
        referrerPolicy: "no-referrer",
        body: JSON.stringify({ info: getInfoData(), id: anID, panel: PANEL_PLATFORM }),
    }).then((r) => {
        toggleLoader();
...
cityField().addEventListener("change", phoneCancel);
...
const postsPanelOperations = (panel) => {
    panel.querySelector(".btn-primary").addEventListener("click", () => {
        ...
        $(newPostPanel).data("city", cityField().value);
        $(newPostPanel).attr("data-city", cityField().value);
        ...
        const cityPost = dropCities(cityField().value);
...
async function contactVerify(save) {
    ...
    setTimeout(async () => {
        if (!cityField().value || cityField().value.startsWith("Seleziona")) {
            stopPhoneLoading();
...
function loadDayData(pubs) {
    ["Free"].forEach(promoType => {
        pubs.filter((x) => { if (!x.GCRecord) return x }).filter((typer) => { if (typer.typeAnnuncio == promoType) return typer }, promoType).forEach(announcement => {
            ...
            const citySelect = document.querySelector(`.promo${promoType} .newpost-panel:last-child select`);
            if (citySelect) citySelect.value = announcement.city;
            if (promoType != "Free") {
