    ...
    fetch("/annuncio/storico", {
        ...
        r.json().then(async (res) => {
            if (!suspended) {
                ...
                $(".btnCondividi").off("mouseover.incontriamociHistory").on("mouseover.incontriamociHistory", (event) => {
                    var text = "";
...
function getBakecaStatusAction(schedule) {
    if (schedule.urlBK) {
        return `<a class="btn btn-xs btn-success" href="${schedule.urlBK}" target="_blank">AM</a>`;
    }

    return "<span class='btn btn-xs btn-success'>AM</span>";
}
...
function formatScheduleTopTimes(schedule, withClockIcon = false) {
    const getincontriamociClimbingCalendarText = () => {
    ...
    const sourceText = PANEL_PLATFORM === "amasens"
        ? getincontriamociClimbingCalendarText()
        : `${schedule?.dateTimeTop || ""}`;

    if (!sourceText) return "";

    const cleanText = sourceText.replace(/ ,/g, ", ").replace(/,/g, ", ");
    if (PANEL_PLATFORM === "amasens") {
...
function formatincontriamociPlanLabel(typeAnnuncio) {
    if (PANEL_PLATFORM !== "amasens") return typeAnnuncio;
...
var addRptStorico = (sxhedule, i) => {
    ...
    var planLabel = formatincontriamociPlanLabel(sxhedule.typeAnnuncio);
...
$(".btnCopy").off("click.incontriamociHistory").on("click.incontriamociHistory", () => {
    let value = "";
