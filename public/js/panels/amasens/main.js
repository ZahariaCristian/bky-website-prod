...
const PANEL_PLATFORM = `${QUERY_PANEL || "amasens"}`.toLowerCase();
...
document.querySelector("#caricalink-button").addEventListener("click", () => {
    ...
    console.log("scrape amasens.com")
    toggleLoader();
...
const loadAnnuncio = (id) => {
    if (id) {
        fetch("/annuncio/getByID", {
            ...
            //document.querySelector(".carica-section").remove();
            document.querySelector("#content h1").innerHTML = "<i>Gestisci l'Annuncio-Amasens</i>";
        });
    }
};
