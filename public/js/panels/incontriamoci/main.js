...
const PANEL_PLATFORM = `${QUERY_PANEL || "incontriamoci"}`.toLowerCase();
...
$(() => {
    //This is New Panel
    if (QUERY_NEW == "new") {
        $(".widget-annuncio .form-control, .widget-annuncio .form-check-input, .widget-annuncio .custom-select, .servizi-box input, .incontriamoci-tags-section input").prop('disabled', false);
    } else {
        if (QUERY_DAY) {
...
document.querySelector("#caricalink-button").addEventListener("click", () => {
    ...
    console.log("scrape incontriamoci.com")
    toggleLoader();
    fetch("/annuncio/scrapeincontriamoci", {
        method: "POST",
        mode: "cors",
        cache: "no-cache",
        credentials: "same-origin",
...
const loadAnnuncio = (id) => {
    if (id) {
        fetch("/annuncio/getByID", {
            ...
            //document.querySelector(".carica-section").remove();
            document.querySelector("#content h1").innerHTML = "<i>Gestisci l'annuncio-incontriamoci</i>";
        });
    }
};
...
const setincontriamociTagGroupValues = (group, values = []) => {
    const wanted = new Set((Array.isArray(values) ? values : [])
        .map(normalizeincontriamociTagValue)
        .filter(Boolean));

    document.querySelectorAll(`input[data-incontriamoci-tag-group='${group}']`).forEach((input) => {
        input.checked = wanted.has(normalizeincontriamociTagValue(input.dataset.incontriamociTag));
    });
};
