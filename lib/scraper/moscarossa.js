...
const PUBLIC_AD_PATH = /^\/girl-(\d+)\.php\/?$/i;
...
function parseMoscarossaAdUrl(value) {
    ...
    const pathMatch = parsed.pathname.match(PUBLIC_AD_PATH);
    if (!["http:", "https:"].includes(parsed.protocol) || !MOSCAROSSA_HOSTS.has(hostname) || !pathMatch) {
        throw new Error("Sono accettati soltanto link pubblici Moscarossa nel formato https://www.moscarossa.biz/girl-123.php");
    }

    const remotePostID = pathMatch[1];
    return {
        remotePostID,
        url: `https://www.moscarossa.biz/girl-${remotePostID}.php`
    };
}
...
async function scrape(value) {
    ...
    try {
        ...
        if (finalUrl.remotePostID !== requested.remotePostID) {
            throw new Error("Moscarossa ha reindirizzato verso un annuncio diverso da quello richiesto.");
        }
        await page.waitForSelector("#p_titolo, .escort-dett-container", { timeout: 15000 });

        const data = await page.evaluate((expectedRemoteId) => {
            ...
            const currentFolder = `/${expectedRemoteId}/`;
            const images = Array.from(document.querySelectorAll("#div_info .foto_escort img[src-big], #div_info .foto_escort img[data-src]"))
                .map((image) => absoluteUrl(image.getAttribute("src-big") || image.getAttribute("data-src") || image.getAttribute("src")))
                .filter((url) => {
                    try {
                        const parsed = new URL(url);
                        return parsed.hostname === "fotom.b-cdn.net" && parsed.pathname.includes(currentFolder) && !/anteprima\.jpg/i.test(parsed.pathname);
                    } catch {
                        return false;
                    }
                });
        ...
        const imageFiles = await downloadMoscarossaImages(browser, data.images, phone, requested.remotePostID);
        return {
            ...data,
            ...requested,
            phone,
