...
const DEFAULT_URL = "https://incontriamoci.xxx/3319878909_i1688105";
  ...
  try {
    ...
    const data = await page.evaluate(() => {
      ...
      const getInfo = (label) => {
        ...
        const match = bodyText.match(new RegExp(`${escaped}\\s*:\\s*([^\\n:]+?)(?=\\s+(?:Categoria|Citt[aà]|Zona|Nome|Et[aà]|Nazionalit[aà])\\s*:|$)`, "i"));
        return clean(match?.[1] || "");
      };

      const images = Array.from(document.querySelectorAll("#item-images .inc-masonry-grid img[src]"))
      ...
      const revealedPhone =
        phoneLinks.map((link) => link.href.replace(/^tel:/i, "")).find(Boolean) ||
        findPhone(document.body.innerText);
      const whatsappNumber = findWhatsappNumber(whatsappLinks, bodyText);
      ...
      return {
        ...
        city: getInfo("Città") || getInfo("Citta"),
        area: getInfo("Zona"),
        name: getInfo("Nome"),
        age: parseInt(getInfo("Eta") || getInfo("Età"), 10),
        nationality: getInfo("Nazionalità") || getInfo("Nazionalita"),
    ...
    const phone = data.phone || parsedUrl.phone;
